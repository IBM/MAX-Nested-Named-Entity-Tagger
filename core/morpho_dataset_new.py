...
class MorphoDataset:
    ...
    def __init__(self, filename=None, input_sentence=None, train=None, shuffle_batches=True, max_sentences=None, add_bow_eow=False, seq2seq=False, bert_embeddings_filename=None, flair_filename=None, elmo_filename=None, inference_mode=False):
    ...
    def _next_batch(self, batch_perm, form_wes_model, lemma_wes_model, fasttext_model, including_charseqs, seq2seq=False):
        ...
        if self._bert_embeddings:
            ...
            for i in range(batch_size):
                #print('i', i)
                #print('np.shape(np.array(batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]]))',
                      #np.shape(np.array(batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]])))

                #print('np.shape(np.array(self._bert_embeddings[batch_perm[i]]))',
                      #np.shape(np.array(self._bert_embeddings[batch_perm[i]])))
                '''print('batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]]',
                      batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]])
                print('self._bert_embeddings[batch_perm[i]]', self._bert_embeddings[batch_perm[i]])'''

                batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]] = \
                    self._bert_embeddings[batch_perm[i]]
        ...
        if self._flair_embeddings:
            ...
            for i in range(batch_size):
                #print('np.shape(np.array(self._flair_embeddings[batch_perm[i]]))', np.shape(np.array(self._flair_embeddings[batch_perm[i]])))
